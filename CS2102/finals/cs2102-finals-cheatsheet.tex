...
\begin{multicols*}{4}
  ...
  \begin{itemize}
    ...
    \item (concurrent execution of a set of transactions is) \definition{serializable} execution is equivalent to some serial execution of the same set of transactions
      \begin{itemize}
        \item ensures \textbf{integrity} of data
  ...
  \begin{itemize}
    ...
    \item \definition{relational database schema} set of relation schemas + data constraints
      ...
      \end{itemize}
    \item \definition{domain} a set of \textit{atomic} values e.g. dom(course) = \{cs2102, cs2030, cs2040\}
      \begin{itemize}
  ...
  \subsection{02-1.1 UNARY OPERATORS}
  ...
  \begin{itemize}
    \item \definition{$\pi_\ell(R)$} projects all attributes of a given \textbf{relation}  specified in list $\ell$
      \begin{itemize}
        \item duplicates removed from output relation!
  ...
  \subsection{02-1.2 SET OPERATORS}
  \begin{itemize}
  ...
  \subsection{02-1.3 CROSS PRODUCT}
  \begin{itemize}
...
CREATE TABLE Uses (
  ...
  \begin{itemize}
    ...
    \item may \textbf{not} preserve all FDs
      \begin{itemize}
  ...
  \textbf{Non-Trivial and Decomposed FD}
  \begin{itemize}
    \item \definition{non-trivial} $\{A\} \rightarrow \{B\}$ where $\{A\} \not\subseteq \{B\}$
    \item \definition{decomposed} $\{A\} \rightarrow \{B\}$ where $B$ is a single attribute
  \end{itemize}

  \subsection{BCNF}
  \begin{itemize}
    \item table in \definition{BCNF} every \textit{non-trivial \& decomposed FD} has a \textit{superkey} as its LHS
    \item \textbf{NOT in BCNF} if $\exists$ non-trivial \& decomposed FD s.t. its LHS is \textit{NOT} a superkey
