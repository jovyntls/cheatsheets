...
\begin{center}
    $V = \pi \int^b_a \left[f(x)\right]^2 \dx \quad$ where $f(x)$ is the radius of the disc
\end{center}
...
\begin{itemize}
    \item compound interest - let $r$ be the interest rate, $A$ be the money
    \begin{itemize}
        \item ODE: $\frac{dA}{dt} = rA; \quad A(0) = C$
        \item solve for $A(t) = Ce^{rt}$
    \end{itemize}
    \item radiocarbon dating - let $\lambda$ be the half life, $C$ be Carbon left
    \begin{itemize}
