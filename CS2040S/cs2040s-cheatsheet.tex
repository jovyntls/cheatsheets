...
\begin{itemize}
    \item $O(\log n)$ - to find the $k^{\text{th}}$ smallest element
...
% \begin{itemize}
%     \item \textbf{simple uniform hashing assumption} - every key has an equal probability of being assigned to every bucket
%     \item \textbf{uniform hashing assumption} - 
% \end{itemize}

% \subsection{chaining}
% \begin{itemize}
%     \item load(hash table), $\alpha = \frac{n}{m}$ = average number of items per bucket
%     \item expected number of items per bucket = load
%     \item time complexity
%     \begin{itemize}
%         \item \code{insert(key, value)} - $O(1 + cost(h))$
%         \item \code{search(key)} 
%         \begin{itemize}
%             \item worst case: $O(n + cost(h))$
%             \item expected case: $O(1 + cost(h))$
%         \end{itemize}
%     \end{itemize}
