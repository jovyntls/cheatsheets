...
\section{SET ADT}
\begin{itemize}
    \item $\checkmark$ speed $\checkmark$ space $\quad\quad \times$ no ordering
    \item $\checkmark$ no false negatives $\quad \times$ may have false positives
    \item \textbf{hash table}: more space, but resolves collisions
\end{itemize}
...
\begin{itemize}
    \item \textbf{breadth-first search} $\then O(V+E)$
    \begin{itemize}
        ...
        \item implement with queue
    \end{itemize}
    \item \textbf{depth-first search} $\then O(V+E)$
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \code{increase/decreaseKey}: bubble up/down leftwards
        \item \code{delete}: swap w bottomrightmost in subtree; bubble down
        \begin{itemize}
            \item \code{extractMax}: \code{delete(root)} 
        \end{itemize} 
    \end{itemize}
    ...
    \begin{itemize}
        \item unsorted array to heap: $O(n)$ (bubble down, low to high)
        \item heap to sorted array: $O(n \log n)$ (extractMax, swap to back)
    \end{itemize}
\end{itemize}

\section{UNION-FIND}
\begin{itemize}
    \item \textbf{union} - connect 2 objects
    \item \textbf{find} - check if objects are connected
