...
Huffman(C):
  ...
  \begin{tightcenter}
    \includegraphics[width=0.8\linewidth]{cs3230-reduction-AB.png} 
  \end{tightcenter}
  ...
  \begin{itemize}
    ...
    \item \textsc{Mat-Sqr}: matrix squaring
      \begin{itemize}
        \item \textit{input}: one $N \times N$ matrix $C$.
        \item \textit{output}: $C \times C$
      \end{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{reduce \textsc{T-Sum} to \textsc{0-Sum}}:
      \begin{itemize}
        \item given array $B$, define array  $A$ such that $A[i] = B[i] - T/2$.
  ...
  \begin{itemize}
    \item \definition{polynomial time} runtime is polynomial in the \textbf{length of the encoding} of the problem instance
      \begin{itemize}
        \item "standard" encodings - binary encoding of integers; list of parameters enclosed in braces (e.g. graphs, matrices)
      \end{itemize}
  ...
  \begin{itemize}
    \item \definition{P} the class of problems solvable in (deterministic) polynomial time
    \item \definition{NP} the class of problems for which polynomial-time verifiable \textbf{certificates} of YES-instances exist.
      \begin{itemize}
