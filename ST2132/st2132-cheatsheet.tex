...
\begin{multicols*}{4}
  ...
  for random draws (without replacement) $x_1,\dots,x_n$: 
  ...
  \begin{itemize}
    ...
    \item if $n << N$, then SRS is like sampling \textit{with replacement}
      (treat the data as if they come from IID RVs $X_1, \dots, X_n$)
  ...
  Let $x_i$ be a realisation of $X_i$. $X_1, \dots, X_{100}$ are random draws with replacement from an imaginary population with mean $w$ and variance $\sigma^2$.
