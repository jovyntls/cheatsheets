...
\begin{multicols*}{3}
  ...
  \definition[, pmf of $X$]{probability mass function} (\textit{discrete}) $\quad p(a) = P(X=a) $
  \begin{itemize}
  ...
  \begin{align*}
    Var(X) &= E[(X-\mu)^2] \\ 
           &= E(x^2) - [E(x)]^2 \\
           &= \sum_{x_i} (x_i - \mu)^2 \cdot p(x_i) \quad\quad \text{(deviation $\cdot$ weight)}
  \end{align*}
  ...
  \textbf{e.g. distribution of time to next event}
  ...
  \begin{niceproof}
    ...
    \\* = $\int^\infty_0 x f_Y(x) \dx$ 
    \\* $= E(Y)$
  \end{niceproof}

  \textbf{N4} - \ildefinition{variance} of $X$, $Var(X) = E[(X - \mu)^2] = E(X^2) - [E(X)]^2$
