...
\begin{multicols*}{3}
  ...
  \textbf{N2} - for a non-negative r.v. $Y$, $E(Y) = \int^\infty_0 P(Y > y) \mathop{dy}$

  \textbf{N3} - if $X$ is a continuous r.v. with pdf $f(x)$, then for any real-valued function $g$, $E[g(x)] = \int^\infty_{-\infty} g(x) f(x) \dx $

  \begin{itemize}
    \item e.g. $E[aX + b] = \int^\infty_{-\infty} (aX + b) \cdot f(x) \dx = a\cdot E(X) + b$
  \end{itemize}
  ...
  $X$ is a \ildefinition{uniform r.v.} on the interval $(\alpha, \beta)$, $\quad X \sim Uniform(\alpha, \beta)$ $\rightarrow$ 
  \begin{tightcenter}
    if its pdf is given by $f(x) = \begin{cases} \frac{1}{\beta-\alpha}, &\alpha<x<\beta \\ 0, &\text{otherwise} \end{cases}$
  ...
  $X$ is a \ildefinition{normal r.v.} with parameters $\mu$ and $\sigma^2$, $\quad X \sim N(\mu,\sigma^2)$ $\rightarrow$
  \begin{tightcenter}
    if the pdf of $X$ is given by $f(x) \frac{1}{\sqrt{2\pi} \sigma}e^{-\frac{1}{2} (\frac{x}{\mu}\sigma)^2}$, $\quad -\infty < x < \infty$
  ...
  a \textit{continuous} r.v. $X$ is a \ildefinition{exponential r.v.}, $\quad X \sim Exponential(\lambda)$ or $Exp(\lambda)$ $\rightarrow$
  \begin{tightcenter}
    if for some $\lambda > 0$ 
    its pdf is given by $f(x) = \begin{cases} \lambda e^{-\lambda x}, &x \geq 0 \\ 0, &\text{otherwise} \end{cases}$
