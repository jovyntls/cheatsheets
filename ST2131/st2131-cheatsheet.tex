...
\begin{multicols*}{3}
  ...
  \\* $\quad$ or $F(a, b) = F_X(a) \cdot F_Y(b)$

  \textbf{N3} - when $X$ and $Y$ are \textbf{discrete r.v.}, the \textit{condition of independence} is equivalent to 
  \begin{tightcenter}
    $P(X=x, Y=y) = P(X=x) \cdot P(Y=y)$ for all $x, y$.
  \end{tightcenter}

  \textbf{N4} - in the \textbf{jointly continuous} case, the \textit{condition of independence} is equivalent to
  \begin{tightcenter}
