...
\begin{multicols*}{3}
  ...
  \begin{tightcenter}
    $P[(X, Y) \in C] = \int\int\limits_C f(x, y) \dx \mathop{dy}$
  ...
  \end{minipage}
  \begin{minipage}[c]{0.55\linewidth}
    \textbf{N1} - if $C = \{(x, y): x \in A, y \in B \}$, then $P(X \in A, Y \in B) = \int\limits_B \int\limits_A f(x, y) \dx \dy$
  \end{minipage}
  ...
  \begin{minipage}[c]{0.65\linewidth}
    ...
    \\* $\quad \approx f(a, b) \mathop{da}\mathop{db}$
