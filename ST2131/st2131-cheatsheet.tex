...
\begin{multicols*}{3}
  ...
  \begin{enumerate}
    ...
    \begin{tightcenter}
      $P(E) = \sum\limits_y P(E \vert Y=y) P(Y=y)$ if $Y$ is \textit{discrete}
      $P(E) = \int\limits^\infty_{-\infty} P(E \vert Y=y) f_Y(y) \dy$ if $Y$ is \textit{continuous}
    \end{tightcenter}

    \begin{niceproof}
      let $X$ be an indicator r.v. for $E$. $\quad \Rightarrow E(X) = P(E)$

      $E(X | Y=y) = P(X=1 \vert Y=y) = P(E \vert Y=y)$
    \end{niceproof}

    \textbf{N5} - find $P((X, Y) \in C)$ given $f(x, y)$: see p.57
    \\* also:  $P(X < Y) = \int P(X < Y \vert Y = y) \cdot f_Y(y)$
    ...
    \definition{Markov's Inequality} if $X$ is a non-negative r.v., for any $a > 0$, $P(X \geq a) \leq \frac{E(x)}{a}$.

    \begin{niceproof}
      let $I$ be an indicator r.v. $=1$ when $X \geq a$. 

      Then $I \leq \frac{X}{a}$, and $E(I) \leq \frac{E(X)}{a}$, and $P(X \geq a) \leq \frac{E(X)}{a}$.
    \end{niceproof}
    ...
    \begin{niceproof}
      $P [ (X - \mu)^2 \geq k^2 ] \leq \frac{E[(X - \mu)^2]}{k^2} $ by Markov's inequality

      Since $(X - \mu)^2 \geq k^2 \iff \vert X-\mu \vert \geq k$, then $P(\vert X - \mu\vert \geq k) \leq \frac{\sigma^2}{k^2}$
    \end{niceproof}

    \textbf{N1} - if $Var(X) = 0$, then $P(X = E[X]) = 1$

    \begin{niceproof}
      let $\mu = E[X]$. by Chebyshev's inequality, for any $n \geq 1$, $P (\vert X - \mu \vert > \frac{1}{n}) \leq \frac{Var(X)}{(\frac{1}{n})^2} = 0 $
